...
- 刷新恢复:历史消息回放、conversation_id 关联项目自动加载(T16)
- 窄屏适配
...
│   │   └── t*-*.mjs          # 浏览器回归走查脚本(T7/T8/T9/T10/T13/T22/T23)
│   └── backend/
...
- **平台 AI 网关故障窗口**(属外部服务故障,非本项目缺陷):上游 AI 供应商返回 403 时,服务端三级降级链路依次为——① 同模型瞬时错误重试 → ② 备用模型通道按序切换(gpt-5.4 → gemini-3.1-pro-preview)→ ③ 全部通道失败后返回 HTTP 502「平台 AI 网关故障,请稍后重试」;前端另有 3 次指数退避自动重试。此时用户侧表现为:自动重试提示(1/3→3/3)→ 失败卡(含网关故障指引文案)。
- **演示模式兜底(显式)**:失败卡出现后,用户可点击「使用演示模式生成」以本地演示智能体完成本次生成——仅由用户主动触发,绝不静默回退;消息带【演示模式】标识、查看器显示演示徽标、项目描述含标记,刷新后可回放。会话与用户消息落库不受网关故障影响。
- 网关恢复后需复测:`test_agent_e2e.py`、`t9-agent-quality-regression.mjs`(真实 AI SSE、项目落库、历史回放、重新生成全链路)。
