...
产出:
  app/backend/reports/t31-reviewer-account.json   账号信息 + 隔离验证证据
  app/backend/reports/t31-reviewer-revoke.sql     禁用/删除评审账号的回收脚本
"""
import json
import os
import secrets
...
REVIEWER_EMAIL = "t31.reviewer@atoms-demo.dev"
REVIEWER_NAME = "T31 验收评审"
VALID_HOURS = 24
...
def main():
    ...
    expires_at = (datetime.now(timezone.utc) + timedelta(hours=VALID_HOURS)).isoformat()
    ...
    rows = mgmt_sql(f"select id, email_confirmed_at is not null as confirmed from auth.users where email = '{REVIEWER_EMAIL}';")
    ...
    status7, write_public = rest("community_apps", token, {
        "method": "POST", "body": json.dumps({"title": "T31 越权写入"}).encode(),
    ...
    status8, created = rest("projects", token, {
        "method": "POST",
        "body": json.dumps({
            "user_id": reviewer_id, "space_id": space_id, "name": "T31 评审样本应用",
            "source": "created", "description": "T31 评审账号自带样本(原子版本 RPC 写入)",
            "app_html": "<!doctype html><html><head><title>T31 Reviewer Sample</title></head>"
                        "<body><h1>T31 Reviewer Sample</h1><p>评审样本应用</p></body></html>",
        }).encode(),
        "headers": {"Prefer": "return=representation"},
    })
    ...
    revoke_sql = f"""-- T31 评审账号回收脚本(评审结束后按需执行)
-- 账号: {REVIEWER_EMAIL}  有效期至(UTC): {expires_at}
...
-- delete from auth.users where email = '{REVIEWER_EMAIL}';

-- 核对
-- select u.email, u.banned_until, (select count(*) from projects p where p.user_id = u.id) as projects
-- from auth.users u where u.email = '{REVIEWER_EMAIL}';
"""
    os.makedirs(REPORT_DIR, exist_ok=True)
    report_path = os.path.join(REPORT_DIR, "t31-reviewer-account.json")
    with open(report_path, "w", encoding="utf-8") as fh:
        json.dump({"steps": steps, "evidence": evidence,
                   "passed": sum(1 for s in steps if s["ok"]), "total": len(steps)},
                  fh, ensure_ascii=False, indent=2)
    with open(os.path.join(REPORT_DIR, "t31-reviewer-revoke.sql"), "w", encoding="utf-8") as fh:
        fh.write(revoke_sql)

    passed = sum(1 for s in steps if s["ok"])
    # 凭据不落仓库:仅写入 /tmp(不纳入版本控制),仓库内报告只保留邮箱、有效期与隔离证据
    creds_path = "/tmp/t31-reviewer-credentials.txt"
    with open(creds_path, "w", encoding="utf-8") as fh:
        fh.write(f"邮箱 {REVIEWER_EMAIL}\n密码 {password}\n有效期至(UTC) {expires_at}\n"
                 f"登录注入 {URL and 'http://127.0.0.1:8899/go'}\n")
    print(f"\nT31 评审账号:{passed}/{len(steps)} PASS")
