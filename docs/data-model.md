...
- 原子写入(T31/T32):RPC `app_write_project_version(p_project_id, p_source, p_label, p_app_html, p_version_number, p_is_demo)`(迁移 009,`security invoker` + `SET search_path = public`)在**单个事务**内完成「取号 → 更新 `projects.app_html` → 插入 `project_versions` 快照」;并发由 `UNIQUE(project_id, version_number)` 兜底,冲突方以最新版本号重试,不产生重复版本号或半写状态;函数内以 `auth.uid()` 校验属主(未登录抛 28000、非属主抛 42501),配合 RLS 使他人项目不可写。线上实测:函数唯一无重载、`authenticated` 具备 EXECUTE、半写 0、版本跳号 0、孤儿快照 0(`app/backend/scripts/t32_rpc_probe.sql`)。
...
- `app/backend/scripts/t31_reviewer_account.py`:评审账号创建与隔离验收 10/10(注册触发器建资料与默认工作区、本人数据 RLS 隔离、越权查询他人数据返回空集、公共表只读、无写策略写入被拒、可创建项目并经原子 RPC 写 v1 快照);结果落盘 `app/backend/reports/t31-reviewer-account.json`。评审账号 `t31.reviewer@atoms-demo.dev` 已于 T32 按 `app/backend/reports/t32-reviewer-revoke-exec.sql` 彻底回收(级联删除资料/工作区/项目/版本,剩余账号 0,脚本可重复执行且幂等)。
